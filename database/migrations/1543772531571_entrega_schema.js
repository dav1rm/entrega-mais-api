...
class EntregaSchema extends Schema {
  up() {
    this.create('entregas', (table) => {
      ...
      table.integer('vendedor_id').unique().unsigned().references('id').inTable('vendedors').onDelete('set null')
      table.integer('endereco_id').unique().unsigned().references('id').inTable('enderecos').onDelete('set null')
      table.integer('status_id').unique().unsigned().references('id').inTable('statuses').onDelete('cascade')
