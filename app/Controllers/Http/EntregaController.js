...
class EntregaController {
  async solicitar({ request, auth, response }) {
    const usuario_atual = auth.user;

    if (usuario_atual.tipo == "e") {
      return response.status(404).send({ message: "Você não é um vendedor" });
    }

    var vendedor_atual = await Vendedor.findByOrFail(
      "usr_vd_id",
      usuario_atual.id
    );
    ...
    if (vendedor_atual.endereco_vend_id == null) {
      return response
        .status(404)
        .send({ message: "Você precisa cadastrar seu endereço" });
    }
    ...
    const dados_produto = request.only(["nome", "valor"]);
  ...
  async minhasEntregas({ auth, response }) {
    const usuario = auth.user;

    switch (usuario.tipo) {
      case "v":
        const vendedor = await Vendedor.findByOrFail("usr_vd_id", usuario.id);
        const entregas_vend = await Entrega.query()
      ...
      case "e":
        const entregador = await Entregador.findByOrFail(
          "usr_et_id",
          usuario.id
        );
        const entregas_ent = await Entrega.query()
      ...
      default:
        return response
          .status(404)
          .send({ message: "Você não possui entregas" });
    }
  }
  ...
  async editar({ request, response }) {
    ...
    if (dados.id == null) {
      return response
        .status(404)
        .send({ message: "Por favor especifique o ID da entrega" });
    }
  ...
  async atualizarStatus({ request }) {
    ...
    const status = await Database.table("statuses").where(
      "entrega_id",
      novo_status.entrega_id
    );
    ...
    for (var i = 0; i < 5; i++) {
      if (status[i].titulo == novo_status.titulo) {
        status_atualizado = await Database.table("statuses")
  ...
  async aceitar({ request, auth, response }) {
    ...
    if (user.tipo == "v") {
      return response.status(404).send({ message: "Você não é um entregador" });
    }
  ...
  async cancelar({ request }) {
    ...
    const affectedRows = await Database.table("entregas")
